...
<div style='text-align:center; padding:20px;'>
    <span style='background-color:#1E3A5F; padding:10px 20px; border-radius:20px; margin:5px;'>
        🔒 개인정보 보호
    </span>
    <span style='background-color:#1E3A5F; padding:10px 20px; border-radius:20px; margin:5px;'>
        🤝 사역 환원 구조
...
if st.button("🔎 AI 추천 받기"):
    ...
    if user_input.strip() == "":
        ...
        with st.spinner("AI가 기도하며 분석 중입니다..."):

            prompt = f"""
            교회 행사 내용: {user_input}

            아래 강사 데이터 중 가장 적합한 3명을 추천하고
            각각의 추천 이유를 간단히 설명해 주세요.
